...
##@ Build
build: pre_build build_cli build_os build_docker build_kubernetes build_cplus  ## Build all scenarios, but it does't include Java scenario
	# tar package
	tar zcvf $(BUILD_TARGET_PKG_FILE_PATH) -C $(BUILD_TARGET) $(BUILD_TARGET_DIR_NAME)

# for example: make build_with cli os_darwin
build_with: pre_build ## Select scenario build, for example `make build_with cli os`
...
build_linux: pre_build ## Build linux version by docker image
...
build_darwin: pre_build build_cli build_os_darwin build_docker build_kubernetes build_java build_cplus ## Build all scenarios darwin version
    # tar package
	tar zcvf $(BUILD_TARGET_PKG_FILE_PATH) -C $(BUILD_TARGET) $(BUILD_TARGET_DIR_NAME)
...
##@ Other
upx: ## Upx compression by docker image, the first time you need to execute `make build_upx_image`
	docker run --rm \
